...
public class InjectUtils {
    ...
    private static void injectEvents(ViewFinder viewFinder, Object object) {
        ...
        for (Method method : methods) {
            OnClickInject onClick = method.getAnnotation(OnClickInject.class);
            if (onClick != null) {
    ...
    private static void injectLayout(Activity activity) {
        //获取我们自定义类ContentViewInject 上面的注解
        Class<?> myClass = activity.getClass();
        ContentViewInject myContentView = myClass.getAnnotation(ContentViewInject.class);
